...
Meteorite_Landings2 <- read_csv("~/Desktop/meteorite-landings.csv")
...
meteorites$year <- suppressWarnings(
  as.numeric(substr(meteorites$year, 1, 4))
)
      ...
      tabsetPanel(
        ...
        tabPanel(
          ...
          fluidRow(
            column(
              12,
              leafletOutput("meteoryt_map", height = 500)
            )
          )
        ),
        ...
        tabPanel(
          ...
          fluidRow(
            column(
              12,
              plotOutput("plot_mass_dist", height = 400)
            )
          )
        ),
        ...
        tabPanel(
          ...
          fluidRow(
            column(
              12,
              plotOutput("plot_year_hist", height = 400)
            )
          )
        ),
...
server <- function(input, output, session) {
  ...
  output$stat_valid_count <- renderText({
    data() %>%
      filter(nametype == "Valid") %>%
      nrow()
  })
  
  output$stat_mean_mass <- renderText({
    format(
      round(mean(data()$mass_g, na.rm = TRUE), 1),
      big.mark = " "
    )
  })
  
  output$stat_max_mass <- renderText({
    format(
      max(data()$mass_g, na.rm = TRUE),
      big.mark = " "
    )
  })
  
  output$stat_min_year <- renderText({
    min(data()$year, na.rm = TRUE)
  })
  
  output$stat_max_year <- renderText({
